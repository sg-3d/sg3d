...
SCAN_DATA = {'ScanNet': {}, '3RScan': {}, 'MultiScan': {}, 'ARKitScenes': {}, 'HM3D': {}}
...
class SceneVerseBase(Dataset):
    ...
    def _load_split(self, cfg, split):
        if self.dataset_name == '3RScan':
            ...
            split_file = os.path.join(self.base_dir, f'MultiScan/annotations/splits/{split}_split.txt')
            scan_ids = {x.strip() for x in open(split_file, 'r', encoding="utf-8")}
            scan_ids = sorted(scan_ids)
        elif self.dataset_name == 'ARKitScenes':
            split_file = os.path.join(self.base_dir, f'ARKitScenes/annotations/splits/{split}_split_non_overlap.txt')
            scan_ids = {x.strip() for x in open(split_file, 'r', encoding="utf-8")}
            scan_ids = sorted(scan_ids)
        elif self.dataset_name == 'HM3D':
            split_file = os.path.join(self.base_dir, f'HM3D/annotations/splits/{split}_split_non_overlap.txt')
            scan_ids = {x.strip() for x in open(split_file, 'r', encoding="utf-8")}
            scan_ids = sorted(scan_ids)
        else:
    ...
    def get_scene(self, scan_id, tgt_object_id_list, tgt_object_name_list, sentence):
        ...
        try:
            ...
            tgt_object_id_list = [0 for tid in tgt_object_id_list]
            print(f"{scan_id} does not has object id overflow")
        # build target info 
